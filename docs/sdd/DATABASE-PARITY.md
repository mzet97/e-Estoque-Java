...
| (nova) "EVENT_PUBLICATION" | eventos (registry Modulith) | ➕ nova | não conflita com schema .NET; identificadores citados; ver ADR-012 |

Regras: Flyway `V1__baseline.sql` = schema .NET byte-a-byte (nomes case-sensitive entre aspas, índices e FKs com mesmos nomes .NET: PK_, FK_, IX_). `ddl-auto: validate`. Uma API Java deve operar sobre um banco criado pelas migrations .NET e vice-versa (exceto a tabela adicional do registry criada por V2). `.gitattributes` mantém as migrations em LF (checksum do Flyway é sensível a line-ending).
